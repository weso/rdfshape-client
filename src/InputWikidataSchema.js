...
function InputWikidataSchema(props) {
    ...
    const initialStatus = { url: '', number: '', rawUrl: '' , content: '', loading: false, error: null}
    const [status, dispatch] = useReducer(reducer, initialStatus);

    useEffect(() => {
            const fetchData = async () => {
                console.log(`Request to ${status.url}`)
                dispatch({type: 'setLoading'} )
                try {
                    const result = await axios(API.wikidataSchemaContent, { params: { wdSchema: status.number }});
    ...
    return (
        <div>
           <InputGroup className="mb-3">
                <InputGroup.Prepend><InputGroup.Text id="entityNumber">{props.name}</InputGroup.Text></InputGroup.Prepend>
            ...
            { status.content ? <details><Code mode='ShExC' value={status.content} readonly={true} /></details>: null }
        </div>
    );
}
